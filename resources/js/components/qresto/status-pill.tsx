...
const statusStyles: Record<OrderStatus, string> = {
    placed:
        'bg-[var(--status-placed-bg)] text-[var(--status-placed-fg)]',
    pending:
    ...
    served:
        'bg-[var(--status-served-bg)] text-[var(--status-served-fg)]',
    paid:
...
export function StatusPill({ status }: StatusPillProps) {
    return (
        <span
            className={`inline-flex h-7 items-center gap-1.5 rounded-full px-3 text-caption font-semibold ${statusStyles[status]}`}
            data-status={status}
        >
            <span
                aria-hidden="true"
